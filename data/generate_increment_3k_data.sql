...
-- ================================================================
-- 1. Generate 3,000 NEW Patients (IDs 10201 - 13200)
-- ================================================================
...
SELECT 
    n + 10200 AS patient_id, -- distinct IDs (shifted to avoid collisions with injected data)
    CASE
    ...
    CONCAT('GHA-', LPAD(n + 10200, 7, '0')) AS mrn
FROM numbers;

-- ================================================================
-- 2. Generate 3,000 NEW Encounters (IDs 10201 - 13200)
...
SELECT 
    n + 10200 AS encounter_id,
    n + 10200 AS patient_id,          -- Maps 1:1 to new patients (offset to avoid collisions)
    ((n - 1) % 1000) + 1 AS provider_id, -- Reuses existing 1000 providers
    encounter_type,
    encounter_date,
...
-- ================================================================
-- 3. Generate 3,000 NEW Billing Records (IDs 10201 - 13200)
-- ================================================================
...
-- ================================================================
-- 4. Generate 3,000 NEW Encounter Diagnoses (IDs 10201 - 13200)
-- ================================================================
...
SELECT 
    n + 10200 AS encounter_diagnosis_id,
    n + 10200 AS encounter_id,
    ((n - 1) % 10000) + 1 AS diagnosis_id, -- Reuse existing 10,000 diagnoses
    (n % 5) + 1 AS diagnosis_sequence
FROM numbers;

-- ================================================================
-- 5. Generate 3,000 NEW Encounter Procedures (IDs 10201 - 13200)
-- ================================================================
...
SELECT 
    n + 10200 AS encounter_procedure_id,
    n + 10200 AS encounter_id,
    ((n - 1) % 10000) + 1 AS procedure_id, -- Reuse existing 10,000 procedures
    DATE_ADD('2025-01-01', INTERVAL (n % 365) DAY) AS procedure_date
FROM numbers;

SELECT 'Incremental data load complete: 3,000 new records added (IDs 10201-13200)' AS status;
