...
-- =====================================================
-- SECTION 3.5: UPDATE SUMMARY DIMENSION (INCREMENTAL)
-- Purpose: Maintain diagnosis-procedure summary dimension
-- Strategy: UPSERT - Update existing pairs, Insert new pairs
-- Runs: Each ETL batch (always fresh data)
-- Performance: Optimizes Query 2 from ~177ms to ~5-10ms
-- =====================================================
START TRANSACTION;
INSERT INTO etl_log (etl_step, status)
VALUES (
        'update_dim_diagnosis_procedure_summary',
        'RUNNING'
    );
SET @summary_log_id = LAST_INSERT_ID();
-- Create temp table with new/updated pairs from this ETL batch
DROP TEMPORARY TABLE IF EXISTS temp_new_summary_pairs;
CREATE TEMPORARY TABLE temp_new_summary_pairs AS
SELECT bd.diagnosis_key,
    ...
    dp.cpt_code,
    dp.cpt_description,
    COUNT(DISTINCT fe.encounter_key) AS new_encounter_count,
    COALESCE(SUM(fe.total_allowed_amount), 0) AS new_total_revenue,
    COALESCE(AVG(fe.length_of_stay_days), 0) AS new_avg_los
...
WHERE fe.created_date >= @etl_start_time -- Only process newly loaded encounters
GROUP BY bd.diagnosis_key,
    bp.procedure_key,
    dd.icd10_code,
    dd.icd10_description,
    dp.cpt_code,
    dp.cpt_description;
-- UPSERT: Update existing pairs or insert new ones
INSERT INTO dim_diagnosis_procedure_summary (
        diagnosis_key,
        procedure_key,
        icd10_code,
        icd10_description,
...
SELECT diagnosis_key,
    ...
    new_total_revenue,
    new_avg_los
FROM temp_new_summary_pairs ON DUPLICATE KEY
UPDATE encounter_count = encounter_count +
VALUES(encounter_count),
    total_revenue = total_revenue +
VALUES(total_revenue),
    avg_length_of_stay_days = (
        (
            avg_length_of_stay_days * encounter_count +
            VALUES(avg_length_of_stay_days) *
            VALUES(encounter_count)
        ) / (
            encounter_count +
            VALUES(encounter_count)
...
SET @summary_rows_affected = ROW_COUNT();
DROP TEMPORARY TABLE temp_new_summary_pairs;
COMMIT;
-- Update log
UPDATE etl_log
SET end_time = NOW(),
    status = 'SUCCESS',
    rows_affected = @summary_rows_affected
WHERE log_id = @summary_log_id;
-- =====================================================
